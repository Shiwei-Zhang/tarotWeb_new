...
$(function(){
    ...
    window.addJoinRecord = function(tableOptions,tableAcolumndoms){
        var id = $('.related_table').length;
        var dom =
            $(
                '<div class="related_table t_' + id + '"> <div class="tables">' +
        ...
        dom.append(dom3);
        dom.find('.template_join_r .condition1').html(tableAcolumndoms);
        dom.find('.template_join_r .condition3').html(tableAcolumndoms);
        $('.joins').append(dom);
        $('.join_plus').unbind('click').click(function(event){
            var currentTable = $(this).parent().parent();
