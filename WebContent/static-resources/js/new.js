	...
	$.ajax({
		...
		success:function(data){
			...
			for(var i=0 ;i<data.length ; i++){
				...
				if(powerConjunction(data[i].POWER,powerEnum2[0])){
				$('#table2').append('<tr name="powerData" class="borderRight"><td>'+data[i].SOURCE_TYPE+'</td><td value="'+data[i].NAME+'">'+data[i].NAME+'</td><td>'
						...
						+'<div class="task"><div class="look hide'+ data[i].ID +'" data-id=' + data[i].ID +' data-name='+data[i].NAME+ '>查看</div>'
						+'<div class="power modifyPower'+ data[i].ID +'" data-id=' + data[i].ID + '>权限管理</div>'
						+'<div class="delete hide'+ data[i].ID +'" data-id=' + data[i].ID + '>删除</div></div></div>'+'</td></tr>');
					
					if(!powerConjunction(data[i].POWER,powerEnum2[1])){
						$('.modifyPower'+ data[i].ID +'').hide();
					}
				}
...
function generateFun(power, domCls, e){
	var id = '123456';
	$('#dropdown-' + id).remove();
    ...
    window.setTimeout(function(){
    	var offset = ($(document).width() - $('.f-header').width())/2;
    	$('#dropdown-' + id).css({
			top: e.pageY + 10,
			left: e.pageX - offset - 5
		}).show();
    }, 10);
} 