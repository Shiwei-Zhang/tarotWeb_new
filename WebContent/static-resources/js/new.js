...
$(function(){
    ...
    function headerActive(){
        if($('#manag').css('display')=='none' && $('.module-data').css('display')=='block'){
            $('.managers').removeClass('header_active');
            $('.flows').addClass('header_active');
        }else if($('.dataManager').css('display')=='block'){
            $('.managers').addClass('header_active');
            $('.flows').removeClass('header_active');
        }
    };
  ...
  //点击确定
        ...
        $('.sureEvent').unbind('click').click(function () {
        	...
        	if(conText=="数据源"){
    			...
    			$('.paramL .paramMain li:gt(0)').each(function(index,item){
    				...
    				var type = $(this).find('span').eq(1).html();
        	...
        	$.ajax({
    	        ...
    	        success:function(data) {
    	        	...
    	        	$(data).each(function(index,item){
    					...
    					tableColumnsMap.put(sourceDataId,item.columnList);
    				});		
    ...
    window.showType=function ($idName, $dataType){
        ...
        if($dataType == 2){
            ...
            $('.flow-template').show();
            //设置属性input宽度
            $('.module-prop .prop input').css('width',250-$('.module-prop .prop span:first-child').width());
        }else if($dataType == 4){
  ...
  //数据连接管理
	...
	var personalInfo=function(){
	    //个人信息、更改密码选项卡
	    $('.btnEvent li').unbind('click').click(function(){
...
function lookSouceData(sourceId){
	var column = new Array();
	var datainfo = new Array();
	$('.columnSd').show();
	$('.mask').show();
	$('.columnSd .closeA,.columnSd .closeC').on('click',function(){
        $('.columnSd').hide();
        $('.mask').hide();
    });
	$.ajax({
	...
	$.ajax({
		...
		success:function(data){
			...
			for(var i=0 ;i<data.length ; i++){
				...
				+'<td style="width:252px;">'+data[i].eMail+'</td>'
				+'<td>'+data[i].createUser+'</td>'
				+'<td style="width:186px;">'+createTime+'</td>'
				+'<td style="width:186px;">'+updateTime+'</td>'
				+'<td class="status">'+isActivation+'</td>'
				+'<td>'+isSystemuser+'</td>'
