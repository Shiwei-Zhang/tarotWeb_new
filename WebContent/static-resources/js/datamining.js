...
var renderPage = function(dataMining, dataMiningTrainInfo, targetInfo, connectionInfo, sourceData){
	//训练模型 和属性信息
	//trainType_ = dataMining.type;
	//con_ = connectionInfo.id;
...
$('.datamining-test').unbind().click(function(){
	...
	var interfaceName = $('.datamining-train .interfaceName').val();
	_trainType = $('.train-type select').val();
	_sourceType = $('.source-type select').val();
	_con = $('.datasource-connection select').val();
	_tableName = $('.datasource-tables select').val();
	_column = $('.datasource-columns select').val();
	/*if(trainType_ != _trainType || sourceType_ != _sourceType || con_ != _con || tableName_ != _tableName || column_ != _column || interfaceName_ != _interfaceName){
		$.dialog('请确认信息是否保存成功');
		return;
	}*/
