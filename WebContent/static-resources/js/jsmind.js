...
(function($w){
    ...
    jm.prototype={
        ...
        add_node_p:function(nodeid,element){
        	...
        	if(this.options.container=="jsmind_container"){
        		level = 1;
            	data = {"level":level,"pk":pk,"sql":"","isSave":0,"isInterface":0,"isDebug":0,"alias":topic,"flowid":flowid,"sourceid":"","subMind":"","columns":"","params":"","line_color":line_color};
        		...
        		//显示屏障
        		$(".mask").show();
        		//因为不能删除z-index属性，只好修改position属性，达到相同的效果，即让showTip显示在最高层
        		$("jmnodes").css("position","static");
        		//定义showTip的位置，再让其显示
        		var jqueryDom = $('#showTip_'+this.options.container+'');
        		jqueryDom.attr('nodeid',nodeid);
        		jqueryDom.css('top',element.offsetTop+element.clientHeight);
        		jqueryDom.css('left',element.offsetLeft);
        		jqueryDom.css('visibility','visible');
         		$('#showTip_'+jm.options.container+' .cancel').unbind('click').click(function(){
        			jqueryDom.css('visibility','hidden');
        			$('.mask').hide();
        			$("jmnodes").css("position","absolute");
        		});
        		$('#showTip_'+jm.options.container+' .sure').unbind('click').click(function(){
        			jqueryDom.css('visibility','hidden');
        			$('.mask').hide();
        			$("jmnodes").css("position","absolute");
        			level = 3;
...
// ============= data provider =============================================
    ...
    jm.view_provider.prototype={
        ...
        add_node:function(node){
            ...
            KeywordMap.put("tableName","");
            var joinList = new Array();
            KeywordMap.put("join",joinList);
            var filterList = new Array();
            KeywordMap.put("filter",filterList);
            var groupList = new Array();
            KeywordMap.put("filter",groupList);
            var orderList = new Array();
            KeywordMap.put("order",orderList);
            var targetList = new Array();
            KeywordMap.put("target",targetList);
            paramsMap.put(pk,KeywordMap);
            //init columnsMap
            var subAsArray = new Array();
            paramsMap.put(pk,subAsArray);
            //init subtreeMap
            ...
            if(!!node){
				...
				if(node.data.level=="1"||node.data.level=="2"){
					...
					if(this.selected_node){
						...
						//保存子树状态
						subtreeMap.put(this.selected_node.data.pk,_jm2.data.get_data('node_array'));
					}	
					/**********************以上是存储的操作，以下是查询展现的操作*******************************/
					open_detail(node.data.pk);
					/*$(".right-slider .subAlias").html("");
					open_detail(node.data.pk);
					var paramsArray = paramsMap.get(node.data.pk);
					var columnArray = new Array();
					var children = node.children;
					//如果是末节点，则columnArray的数据来自数据源的字段，否则来自"该节点的别名.子节点的as"的列表总和
					if(node.data.level==1){
						...
						for(var i in children){
							...
							var dataArray = subtreeMap.get(child.data.pk).data;
							for(var j in dataArray){
								if(dataArray[j].parentid==child.data.pk){
									var obj = new Object();
									obj.name = paramsMap.get(child.data.pk)[4]+"."+dataArray[j].subAlias;
									//这里对字段类型先暂时这么定义
									obj.type = "varchar";
									columnsTemp[j] = obj;		
								}
					...
					//显示columnList页面，以及赋值filterStr和groupStr
					var columnsHtml = "<ul ><li >字段：</li>";
					var filterStr = "<option value=''>选择字段</option>";
					if(columnArray&&columnArray.length>0){
						var columnName = null;
						for(var i in columnArray){
							//包含name,type
							columnName = columnArray[i].name;
							columnsHtml +="<li class = 'columnLi'  onmouseover='showTooltip(this);' jm = '2' textValue = '"+columnName+"' level='4' id='"+columnName+"' draggable='true' ondragstart='drag(event)'><a>"+columnName+"</a></li>";
							filterStr+="<option value='"+columnName+"'>"+columnName+"</option>";
						}
						columnsHtml +="</ul>";		
					}
					//document.getElementById("columnList").innerHTML = columnsHtml;
					
					//2.加载collapse_group内容
					var groupHtml = "<div class='group'><select class='condition1'>"+filterStr+"</select>"+
	                   				"<span class='trash' style='display:none;'><i class='fa fa-trash'></i></span>"+
	                   				"</div>";
					$('#collapse_group .groups').html(groupHtml);*/
					/*if(paramsArray[2].length>0){
						for(var i=0;i<paramsArray[2].length-1;i++){
							addRecord('GROUP');
						}
						var j = 0;
						$('.groups .group').each(function(){
							$(this).find(".condition1").val(paramsArray[2][j]);
							j++;
						});
					}
					//3.加载collapse_order内容
					var orderHtml = "<div class='order'><select class='condition1'>"+filterStr+"</select>"+
			                   ...
			                   "<span class='trash' style='display:none;'><i class='fa fa-trash'></i></span>"+
			                   "</div>";
					$('#collapse_order .orders').html(orderHtml);

					if(paramsArray[3].length>0){
						for(var i=0;i<paramsArray[3].length-1;i++){
							addRecord('ORDER');
						}
						var j = 0;
						$('.orders .order').each(function(){
							$(this).find(".condition1").val(paramsArray[3][j].name);
							$(this).find(".condition2").val(paramsArray[3][j].sort);
							j++;
						});
					}
					//4.加载alias内容
					var aliasHtml = "<label>"+node.topic+" AS <input id='aliasInput' type='text' value='"+paramsArray[4]+"' /></label>";
					$('.right-slider .alias').html(aliasHtml);
					//1.加载collapse_filter内容，在ie下collapse_filter需要后加载，暂时不清楚原因
					var dom = $('<div class="filter">' +  $('#filterModule').html() + '</div>');
					dom.find('.condition1').html(filterStr);
			        $('.filters').html(dom);
					//$('#collapse_filter .filters').html(filterHtml);
					if(paramsArray[1].length>0){
						for(var i=0;i<paramsArray[1].length-1;i++){
							addRecord('FILTER');
						}
						var j =0;
						$('.filters .filter').each(function(){
							$(this).find(".condition1").val(paramsArray[1][j].name);
							$(this).find(".condition2").val(paramsArray[1][j].code);
							$(this).find(".condition3").val(paramsArray[1][j].value);
							$(this).find(".condition4").val(paramsArray[1][j].conn);
							$(this).find(".condition5").val(paramsArray[1][j].bracket);
							j++;
						});
					}
					//5.加载collapse_join内容
					...
					if(children.length>1){
						var tableArrayStr = "<option value=''>选择表</option>";
						for(var i=0;i<children.length;i++){
							var childrenParamsArray = paramsMap.get(children[i].data.pk);
							tableArrayStr+="<option value='"+childrenParamsArray[4]+"'>"+childrenParamsArray[4]+"</option>";
						}
						for(var i=0;i<children.length-1;i++){
							var id = jsMind.util.uuid.newid();
							var dom = $('<div id = '+id+' class="big_join">' +  $('#joinModule').html() + '</div>');
							var hrefId = jsMind.util.uuid.newid();	
							//用来设定每次一个小折叠的点击事件
							dom.find('.joinClass').attr("id",hrefId);
							//这个class很重要，模板的class没有折叠效果
							dom.find('.joinClass').attr("class","panel-collapse collapse in");
							dom.find('.collapse_join_body').attr("data-target","#"+hrefId);
							dom.find('.condition').last().html(tableArrayStr);
							dom.find('.condition').first().html(tableArrayStr);
					        $('.big_joins').append(dom);
					        //需要手动定义事件，不定义会报too much recursion
					        $('#'+hrefId).on('show.bs.collapse', function(e){
					        	//console.log(hrefId);
								$('#'+id+' .collapse_join_icon>i:first-child').hide();
								$('#'+id+' .collapse_join_icon>i:last-child').show();
								e.stopPropagation();
							});
							$('#'+hrefId).on('hide.bs.collapse', function(e){
								$('#'+id+' .collapse_join_icon>i:first-child').show();
								$('#'+id+' .collapse_join_icon>i:last-child').hide();
								e.stopPropagation();
							});
						}
						//初始化值
						if(paramsArray[0].length>0){
							var j =0;
							$('.big_joins .big_join').each(function(){
								if(paramsArray[0][j]){
									$(this).find(".condition").first().val(paramsArray[0][j].Ltable);
									$(this).find(".condition").last().val(paramsArray[0][j].Rtable);
									$(this).find(".condition")[1].value=paramsArray[0][j].conn;
									var bigjoinId = $(this).attr("id");
									var retailArray = paramsArray[0][j].retail;
									if(retailArray.length>0){
										for(var i=0;i<retailArray.length-1;i++){
											addRecord('JOIN',bigjoinId);
										}
										var i =0;
										$('#'+bigjoinId+' .joins .join').each(function(){
											$(this).find(".condition1").val(retailArray[i].Lcolumn);
											$(this).find(".condition2").val(retailArray[i].code);
											$(this).find(".condition3").val(retailArray[i].Rcolumn);
											$(this).find(".condition4").val(retailArray[i].conn);
											$(this).find(".condition5").val(retailArray[i].bracket);
											i++;
										});
									}
									j++;
					...
					//6.showSql按钮
					var showSqlHtml = "<label><button onclick='show_sql();'>show sql</button></label>";
					$('.showSql').html(showSqlHtml);*/
				}else if(node.data.level=="3"||node.data.level=="4"||node.data.level=="5"){
					//每次都清除一下主树的选中点，不改变式样的清除选中点，确保快捷键能正常使用
					_jm.select_clear_fake();
					//if(this.selected_node&&this.selected_node!=node){	
					if(this.selected_node){
						//当之前做了删除节点的操作后，document.getElementById("subAlias")会变为null
						if(document.getElementById("subAliasInput")){
							//如果当前选中的节点是二级节点，那么需要修改子树的as列表
							//好像不需要再添加下面的条件，因为如果document.getElementById("subAlias")不为null，那么当前选中的节点一定是二级节点，因为只有二级节点才有subAlias
							if(this.selected_node.parent&&this.selected_node.parent.isroot){
								this.selected_node.data.subAlias = document.getElementById("subAliasInput").value;
							}
						}
						//保存一下常量的值
					...
					if(node.parent.isroot){
						if(node.data.level==5){
							subAliashtml = "<label>value:<input name='constantValue' type='text' id='constantValue' value='"+node.data.constantValue+"' /></label>"+
							"<label>"+node.data.sql+" AS <input id='subAliasInput' type='text' value='"+node.data.subAlias+"' /></label>"+
							"<label><button onclick='show_subSql();'>show subSql</button></label>";
						}else{
							subAliashtml = "<label>"+node.data.sql+" AS <input id='subAliasInput' type='text' value='"+node.data.subAlias+"' /></label>"+
							"<label><button onclick='show_subSql();'>show subSql</button></label>";
						}
					}else{
						if(node.data.level==5){
							...
							subAliashtml = "<label><button onclick='show_subSql();'>show subSql</button></label>";
						}
						
					}	
					//subAliashtml = "<label>"+node.topic+" AS <input id='subAliasInput' type='text' value='"+columnsMap.get(node.parent.id)[node.index-1]+"' /></label>";
					$('.right-slider .subAlias').html(subAliashtml);
				}
...
//            this.handles['right'] = this.handle_right;
        ...
        handle_delnode:function(_jm,e){
            var selected_node = _jm.get_selected_node();
            if(!!selected_node && !selected_node.isroot){
            	//console.log(_jm.options.container);
				//取消删除后自动选中父节点
                //_jm.select_node(selected_node.parent);
                _jm.remove_node(selected_node);
                //如果是主树，需要清理面板。（快捷键好像对两个jsmind都有效，_jm.options.container放在!!selected_node里面就好了，之后可以进一步研究）
