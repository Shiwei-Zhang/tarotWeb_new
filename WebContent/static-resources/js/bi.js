					...
					if(chartFrameId && chartFrameId != 1){
						if(layoutId){
							...
							$('.bi-template-layout').append(
									$('<div class="bi-layout" style="margin-top:5px;" data-layoutid="'+ data.id + '" data-templateid="' + data.templateId +'" data-chartframegroupid="'+ data.chartFrameGroupId + '" data-height="'+data.height+'" data-width="'+data.width+'" data-theme="'+data.theme+'">'+data.name+'</div>') );
							bindLayoutEvent();
						}
					}else{
						console.log('Nothing to do.');
...
//});
    ...
    var renderLayoutList = function(layout){
    	...
    	$('.bi-template-layout').append(
    			$('<div class="bi-layout" style="margin-top:5px;" data-layoutid="'+ layoutId + '" data-templateid="' + templateId +'" data-chartframegroupid="'+ chartFrameGroupId + '" data-height="'+height+'" data-width="'+width+'" data-theme="'+layout.theme+'">'+name+'</div>') );
    	bindLayoutEvent();
    };
    //chart渲染到页面
  ...
  //点击显示布局弹框
    ...
    var renderLayout = function($borderMaps, $templateContainer){
    	for(var i = 0; i < $borderMaps.length; i++){
			...
			calculateLayout($borderMap, $templateContainer);
		}
    ...
    //根据四条边计算宽高
    var calculateLayout = function($borderMap, $templateContainer){
		...
		var $dom = $('<div class="border-layout"></div>').css({'position':'absolute', 'left':positionLeft.left + 'px', 'top': positionLeft.top + 'px', 'width': width + 'px', 'height': height + 'px', 'border-left':'1px dotted gray', 'border-top':'1px dotted gray'});
		$templateContainer.append( $dom );
		//边线池添加边线
		...
		$centerDom.css({'left':centerPosition.left + 'px', 'top':centerPosition.top + 'px', 'border':'1px solid #b9b9b9'});
