...
//$(function(){
            ...
            success:function(data) {
            	if(data){
					...
					if( chartType == 'input' || chartType == 'droplist' || chartType == 'table'){
						$('#bi-main').createVariable(event, $chartJson, chartType);
					}else if($chartJson){
	...
	$('.bi-filter-reset').unbind().click(function(){
		...
		var filter = $('.bi-filter-container .bi-filter');
		if(filter.length == 1){
			return;
		}
		...
		//按钮重置
		filter.find('i.bi-filter-close').hide();
		filter.find('i.bi-filter-add').show();
		filter.find('div.bi-filter-related').hide();
	});
	...
	$('.bi-title input[name="text"]').unbind().blur(function(){
		...
		if( titleShow == 1 && (chartType == 'input' || chartType == 'droplist' || chartType == 'table') ){
			$('.variable-div').find('.variable-title').text(titleText);
			return ;
		}
	...
	$('.bi-title .alignIcon').unbind().click(function(){
		...
		if($this.hasClass('checked')){
			...
			if( titleShow == 1 && chartType == 'table' ){
				$('.variable-div').find('.variable-title').css('text-align',titlePosition);
				return ;
			}
	...
	var initAttributeUI = function(){
		...
		if(chartType == 'input' || chartType == 'droplist' || chartType == 'table' ){
			console.log('do not need those attributes.');
			return;
		}
		...
		$.ajax({
			...
			success:function(data) {
				if(data){
					...
					var divId = 'tmp-' + chartId + '-' + templateId;
					if( $chartType == 'input' || $chartType == 'droplist'){
						$('.bi-template-container').append( $( '<div id="' + divId + '-parent" style="height:30px;  width:240px; position: absolute;"><div class="tmp-chart" id="' + divId  + '" style="height:30px; width:240px;"></div></div>' ) );	
					}else if( $chartType == 'table' ){
...
//});
	...
	var renderChart = function(chart){
		...
		$('.bi-chart-list').append( 
				$('<div class="bi-edit" style="margin-top:5px;" data-chartid="'+ chartId + '" data-templateid="' + templateId +'" data-conid="'+ conId + '" data-charttype="'+chart.classType+'">'+name+'</div>') );
		//绑定点击和拖拽事件
		biEditBindEvent();
